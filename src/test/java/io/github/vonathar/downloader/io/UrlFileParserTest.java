...
import static io.github.vonathar.downloader.io.UrlFileParserTest.TestUtils.getExpectedUrls;
import static io.github.vonathar.downloader.io.UrlFileParserTest.TestUtils.writeTestUrls;
...
class UrlFileParserTest {
  ...
  static class TestUtils {
    ...
    @SneakyThrows
    static void writeTestUrls(int n, Path urlFile,
      ...
      for (int i = 0; i < n; i++) {
        if (hasBadUrls && i % 3 == 0) {
          ...
          output.append("https://www.goodurl.com/")
              .append(i)
              .append("\n");
        }
    ...
    static Set<URI> getExpectedUrls(int n, boolean hasDuplicates,
        boolean hasBadUrls) {
      Set<URI> expectedUrls = new HashSet<>();
      for (int i = 0; i < n; i++) {
        if (!hasBadUrls || i % 3 != 0) {
          expectedUrls.add(
              URI.create("https://www.goodurl.com/" + i));
        }
        if (hasDuplicates) {
          i++;
        }
